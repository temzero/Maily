...
export const UnsealedMailDetail: Component<SealedMailProps> = (props) => {
    ...
    const recipientName =
        getRecipientDisplayName?.(props.email.to?.[0]) || getSenderDisplayName(props.email.to?.[0]);

    const isSender = senderEmail && currentUser()?.email && senderEmail === currentUser()?.email;
        ...
        >
            ...
            <div class="h-full w-full flex-1 flex justify-between items-end p-2">
                <div class="flex items-center">
                    {isSender && <AiOutlineArrowRight size={40} class="-ml-2 opacity-70" />}
                    ...
                    <div class="flex items-end gap-2">
                        ...
                        <div>
                            <h1 class="text-xl font-semibold font-sans leading-none">
                                {getFirstAndLastName().firstName} {getFirstAndLastName().lastName}
                            </h1>
                            <p class="text-sm opacity-70">{displayEmail}</p>
                        </div>
                    </div>
                </div>

                {/* Created At - Bottom Right */}
                <div class="opacity-60 font-sans text-sm">
