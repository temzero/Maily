...
export const UnsealedMail: Component<SealedMailProps> = (props) => {
    ...
    const getInitials = () => {
        ...
        return (first + last).toUpperCase() || senderEmail().charAt(0).toUpperCase();
    };
    ...
    return (
        ...
        >
            ...
            <div
                id="mail-content"
                class={`bg-white text-black p-2 overflow-auto mb-1 `}
                style={{
                    width: '90%',
                    height: 'calc(100% - 16px)', // Height from top to above envelope
                }}
            >
                <h1 class="text-sm font-bold">{props.email.subject || '(No Subject)'}</h1>
                {/* <p class="text-xs">{props.email.content || '(No Content)'}</p> */}
                <div class="prose max-w-none text-xs" innerHTML={props.email.content} />
            </div>
